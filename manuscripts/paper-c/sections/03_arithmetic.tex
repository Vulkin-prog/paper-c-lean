...
The partition balances two resources: the number of host pairs and the
weight carried by the relation space at each host. The rational mass has
already been separated. For the residual mass, a small dimension makes
many hosts affordable; when the dimension is large, a bounded component
or a defect must instead make the hosts rare. The first tests exploit
these alternatives, with Runge excluding the aligned deep case. In the
terminal sector neither a one-kernel host count nor the full relation
weight suffices separately: several small kernels in one short window
must be counted together. The assembly in \S\ref{sec:profile-assembly}
records the cost obtained from each branch.
...
The uniform polynomial-height Pell estimate is classical: see
\cite[arXiv text, Lemma~4]{CillerueloGaraev2011} and
\cite[Lemma~2.10 and Remark~2.12]{Tao2026}.
We retain a self-contained proof with the explicit height dependence
required here; the lemma is not a separate priority claim.
...
The remaining issue is concentration within a window. On a slice with
larger start of size $X$, one kernel below a constant multiple of
$\sqrt{XB}$ allows $X^{3/4+o(1)}$ candidate starts
(\cref{lem:terminal-partners}); this count is too large when paired with
the full relation weight $Q_B$. If the component constraints force two
such kernels in the same window, the shifted energy
(\cref{lem:kernel-energy}) reduces the count to the
$X^{2/3+o(1)}$ scale. If only one small kernel is forced, the residual
dimension is smaller, and the one-kernel container is sufficient.
\Cref{macro:lem:determinant} supplies the small kernels;
\cref{prop:terminal-final} combines the two populations. The gain comes
from counting their concentration, not from shrinking the container.
...
$B$-smooth integer, with $q\asymp X/R$ and $q'\asymp X/S$.
To count their choices explicitly, put
\[
 \mathscr Q_B(T)=\{a^2u\le T:u\text{ squarefree},\ P^+(u)\le B\}.
\]
The squarefree decomposition is unique, and therefore
\[
 |\mathscr Q_B(T)|
 \le\sum_{\substack{u\le T\text{ squarefree}\\P^+(u)\le B}}
                       \left\lfloor\sqrt{T/u}\right\rfloor
 \le\sqrt T\,W_B.
\]
For suitable absolute constants in the height bounds, the number of
cofactor pairs $(q,q')$ is thus at most
\[
 C\sqrt{X/R}\,W_B\sqrt{X/S}\,W_B
       =\frac{CXW_B^2}{\sqrt{RS}}.
\]
Fix such a pair and write $g=(q,q')$. The equation
\[
 qr+h=q's
\]
requires $g\mid h$. When it has a solution $(r_0,s_0)$, all solutions are
\[
 r=r_0+\frac{q'}g t,\qquad s=s_0+\frac qg t,\qquad t\in\Z.
\]
The box $r\asymp R$, $s\asymp S$ contains at most
\[
 1+C\min\{Rg/q',Sg/q\}\le1+CBRS/X
\]
solutions, since $g\le h\le B$. Dropping any remaining squarefreeness
or coprimality restrictions only enlarges this upper bound. Multiplying
the number of cofactor choices by the number of solutions for each gives
\[
 X^{o(1)}\left(\frac{X}{\sqrt{RS}}+B\sqrt{RS}\right)
 \le X^{2/3+o(1)}.
\]
Here $X/\sqrt{RS}\le X^{2/3}$, while
$B\sqrt{RS}\ll X^{1/2}B^{3/2}=X^{1/2+o(1)}$, as $B\asymp\log X$.
...
\subsection{Assembly and lower bounds}\label{sec:profile-assembly}
