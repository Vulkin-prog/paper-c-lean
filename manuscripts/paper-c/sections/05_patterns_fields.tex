...
The exponent $1/2$ is a sufficient uniform bound, not an optimality claim. The
...
\subsection{Typical dictionaries: average before taking the worst case}
\label{sec:typical-dictionaries}
The uniform theorem protects against an adversarial choice of words. A
randomly chosen dictionary asks a different question: can its particular
selection avoid most of the arithmetic dependence? The useful average is
not the law obtained by forgetting the dictionary. We first fix the
selected words, compare the resulting labelled laws, and only then average
the distances.

For a dictionary $\mathcal W$ and a deterministic start mask $A\subset I_N$,
write
\begin{equation}\label{eq:typical-distance}
 \Delta_{N,A,Y}(\mathcal W)=
 \EE\,\dTV\left(\Law((I_{x,w})_{x\in A,w\in\mathcal W}\mid\mathcal F_Y),
           \bigotimes_{x\in A,w\in\mathcal W}\Pois(2^{-B})\right).
\end{equation}
The expectation in this definition is over the prime environment only.
At $A=I_N$ and $Y=Y_N^\sharp$, abbreviate it by $\Delta_N(\mathcal W)$.

\begin{theorem}[Typical word dictionaries]\label{thm:typical-dictionaries}
Fix $0<\beta_-<\beta_+<\infty$, $0<\lambda_-<\lambda_+<\infty$ and
$0<\varepsilon<1/3$. For all sufficiently large $N$, uniformly for integers $B,m$ with
\[
 \beta_-\log N\le B\le\beta_+\log N,\qquad
 1\le m\le2^B,\qquad \lambda_-\le\Lambda=Nm2^{-B}\le\lambda_+,
\]
choose $\mathcal W$ uniformly among the $m$-element subsets of
$\{\pm1\}^B$, independently of $f$. There is a deterministic bound
\begin{equation}\label{eq:typical-rate}
 \EE_{\mathcal W}\Delta_N(\mathcal W)\le r_N,\qquad
 r_N\ll_{\beta_-,\beta_+,\lambda_-,\lambda_+,\varepsilon}
             e^{-V_N+o(\nu_N)}+N^{-1/3+\varepsilon}.
\end{equation}
The little-oh is uniform in $B$ and $m$. For every $t>0$,
\begin{equation}\label{eq:typical-markov}
 \PP_{\mathcal W}\{\Delta_N(\mathcal W)>t\}\le \min\{1,r_N/t\}.
\end{equation}
Thus $t_N\to0$ and $r_N/t_N\to0$ give a class of dictionaries of
proportion tending to one, each with mean conditional distance at most
$t_N$. Every deterministic restriction of its site--word coordinates obeys the same
bound. Moreover the mean, over $\mathcal W$, of the unconditional
 distance between its multiplicative and independent-sign word fields
is at most $r_N+O(\Lambda^2B/N)$.
\end{theorem}
\begin{proof}
Put $p_B=Q^{-1}$, $Q=2^B$, $a=m/Q$, and
$b=m(m-1)/(Q(Q-1))$. For fixed words $u,v$,
\begin{equation}\label{eq:dictionary-selection}
 \PP_{\mathcal W}(u,v\in\mathcal W)
       =b+(a-b)\1_{\{u=v\}},\qquad 0\le b\le a^2\le a.
\end{equation}
In particular, if $\mathbf f_x=(f(x-1+j))_{0\le j<B}$, independence of the
selection from $f$ gives
\begin{equation}\label{eq:selected-pair}
 \EE_{\mathcal W}\PP_f(\mathbf f_x,\mathbf f_y\in\mathcal W)
       =b+(a-b)\PP_f(\mathbf f_x=\mathbf f_y)
       \le a^2+a\PP_f(\mathbf f_x=\mathbf f_y).
\end{equation}
For disjoint supports, let $\kappa(x,y)$ be the nullity of the $B$
valuation vectors for the ratios of corresponding values. Sending a
coefficient vector $t$ to $(t,t)$ injects its relation space into the
full $2B$-value relation space. Therefore
\begin{equation}\label{eq:word-collision}
 \PP_f(\mathbf f_x=\mathbf f_y)=2^{-B}2^{\kappa(x,y)}
                     \le p_B2^{\rho_B^{\rm val}(x,y)}.
\end{equation}
For good supports at distance $1\le d<B$ and $Y>2B$, all $B+d$ values
in their union are conditionally uniform. Their equality graph has $d$
chains and $B$ independent edges. Thus
$\PP_f(\mathbf f_x=\mathbf f_{x+d}\mid\mathcal F_Y)=p_B$.

Apply the process Chen--Stein bound to each fixed dictionary on each
prime fibre, using the same site--word graph as in
\cref{thm:word-field}. Average this \emph{nonnegative error bound} over
$\mathcal W$. The mean real deletion cost at each bad site is exactly
$a$, regardless of its arithmetic word law; the target cost is at most
$a$. Products of good marginals cost $O(a^2(NB+E_Y))$. Distinct words
at the same site cannot occur together. Equations
\eqref{eq:selected-pair}--\eqref{eq:word-collision} bound all remaining
pairs: their baseline is absorbed since $ap_B\le a^2$, and their
arithmetic excess costs $ap_BR_2^{\rm val}(N,B)$. More generally, for
every deterministic mask $A$ this proves
\begin{equation}\label{eq:typical-finite}
 \EE_{\mathcal W}\Delta_{N,A,Y}(\mathcal W)
 \ll a|D_Y^{\rm val}\cap A|+
 a^2\bigl(|A|B+E_Y(A)\bigr)+ap_BR_2^{\rm val}(A,B).
\end{equation}
Here $E_Y(A)$ counts ordered distinct-site pairs sharing a nonzero valuation-parity
coordinate above $Y$, and $R_2^{\rm val}(A,B)$ sums over ordered pairs
in $A$ at distance at least $B$. All sums may include deleted sites.
The companion, \cref{supp:sec:typical-details}, gives the full finite
bookkeeping. In particular no weighted defect term is removed from the
\emph{uniform} theorem: its removal here uses the dictionary average.

At $Y=Y_N^\sharp$, the deletion and edge bounds are those of
\cref{prop:cutoffs}. The coarse full-value profile gives
\[
 ap_BR_2^{\rm val}(N,B)
 \ll_\varepsilon \Lambda N^{-1/3+\varepsilon}
                           \left(1+\frac{\Lambda}{m}\right).
\]
Together with $a^2NB=\Lambda^2B/N$, this proves
\eqref{eq:typical-rate}. Markov's inequality proves
\eqref{eq:typical-markov}. Restrictions contract the distance for each
fixed dictionary, before either expectation. For independent signs there
are only local edges; the same selection calculation gives mean Poisson
error $O(\Lambda^2B/N)$. Comparing the two fields with their common
Poisson target proves the last assertion.
\end{proof}
For example, $m_N\asymp N$ and $B_N=2\log_2N+O(1)$ are allowed.
More generally $m_N\asymp N^\alpha$ for any fixed $\alpha\ge0$ is
allowed with $B_N=(1+\alpha)\log_2N+O(1)$ and a suitable fixed band.
There is no uniform assertion over all dictionaries of those sizes.
The dictionary is selected independently of the prime signs and then
held fixed; neither adaptation to the observed sequence nor an
almost-sure conclusion over all scales is part of the theorem.

\begin{corollary}[Random affine dictionaries]\label{cor:affine-dictionaries}
Identify sign words with $\F^B$. Choose an $r\times B$ matrix $A_0$
uniformly among the matrices of row rank $r$, where $0\le r\le B$,
and choose $b_0\in\F^r$ independently and uniformly. The random dictionary
\[
 \mathcal W=\{u\in\F^B:A_0u=b_0\},\qquad m=2^{B-r},
\]
is independent of $f$. Under the band and intensity hypotheses of
\cref{thm:typical-dictionaries}, all its conclusions hold for this
ensemble as well. In particular $r=\log_2N+O(1)$ and
$B=2\log_2N+O(1)$ give dictionaries of order $N$ specified by
$r(B+1)=O((\log N)^2)$ bits.
\end{corollary}
\begin{proof}
Each codimension-$r$ kernel has the same number of ordered row bases,
and uniform $b_0$ chooses uniformly among its cosets. A fixed word is
included with probability $a=2^{-r}$. For distinct $u,v$, invariance
under invertible linear maps gives
\[
 \PP(A_0(u-v)=0)=\frac{2^{B-r}-1}{2^B-1},\qquad
 \PP(u,v\in\mathcal W)=2^{-r}\frac{2^{B-r}-1}{2^B-1}
                      =\frac{m(m-1)}{Q(Q-1)}.
\]
These are precisely the one- and two-point probabilities used in
\eqref{eq:typical-finite}; no higher inclusion probability was used.
They transfer the nonnegative deletion, pair and overlap costs, not an
identity between the two ensembles' average total-variation distances.
The dictionary remains fixed while bounding each distance; only its
bound is then averaged over the actual matrix and offset.
The cases $r=0$ and $r=B$ obey the same formulas.
\end{proof}
The corollary supplies a short random description and a high-probability
statement, not a deterministic construction certified at every scale.
The proof uses the one- and two-point sampling identities of the affine
ensemble. A short matrix description by itself does not certify the
Poisson approximation for a chosen dictionary or control its overlap
weight.

The following exact identity controls local overlaps alone. It is useful
for the uniform theorem, but does not replace averaging the full
distance as in \cref{thm:typical-dictionaries}.
...
The uniform and typical dictionary comparisons control the rare occurrences of each
...
The fixed enlarged-cutoff bound remains useful for events exposed up to
$\widehat Y_N$. When their information can be exposed at a smaller
admissible cutoff, \cref{prop:information-labelled} optimizes the two
labelled costs as a function of that information.
