...
$-c\nu+o(\nu)$ and $-2c\nu+o(\nu)$.
Take $E=3\lceil\widehat V_N/\log2\rceil$. Since
$I_N^{\rm info}+\ell\le w\le\widehat V_N$, the enlarged support is
$\log_2N+O(V_N)$, uniformly in the parameters, and the real conditional
and target tails are $O(e^{-2\widehat V_N})$.
This larger auxiliary truncation disappears from the final field. The polynomial remainder is at most
...
$w=\max\{w_0,w_N(I)\}$. Since $w\ge w_0$, the original event
belongs to $\mathcal F_{\lfloor e^w\rfloor}$ by upward inclusion of the
prime sigma-fields; no new measurability assumption is required.
The sufficient condition there is
