...
Assume $n\ge1$, $Q\le n$ and $Y>Q$, as holds eventually in the ambient regime.
...
\end{lemma}
\begin{proof}
Assign each prime of $r$ to a target block and offset where it is hosted. There are at most $((k-1)(Q+1))^{\omega(r)}$ assignments. For a fixed assignment, CRT fixes each target index modulo the product
$r_h$ of its assigned primes. A target block receiving no prime has
probability exactly one. For a nonempty block,
$r_h\le r\le m\le n+Q\le2n$, so that residue has probability at most
$1/r_h+1/n\le3/r_h$. There are at most $\omega(r)$ nonempty blocks;
their nontrivial moduli have product $r$. Independence therefore bounds
the assignment probability by $3^{\omega(r)}/r$, not by a factor
three for every empty block as well. Summing assignments gives
\eqref{supp:palm:eq:crt}.
...
Let $\mathscr R_M$ be the target configurations of mass at most $K_M=\lceil2\Lambda\rceil$, with low types $e\le E_*$, sites in $G_\vartheta$, and regular maximal supports. Equivalently, enumerate all its atoms,
with their multiplicities, and impose regularity on every raw vertex
occurrence of that enumeration. This definition is invariant under
permutation, so it passes from the ordered sample to the point
configuration without losing repeated occurrences. The empty
...
\]
Once the exact planted words are fixed, no further retained atom is,
almost surely, the event that every remaining base-start indicator is
zero. Simultaneous finiteness of the runs gives this equivalence, and
both positive-probability conditionings preserve that full-measure event. Hence
...
$L_z=(-\log v(z)-\Lambda)_+$ on regular configurations when $v(z)>0$,
$L_z=+\infty$ there when $v(z)=0$, and $L_z=0$ otherwise, gives
$(1-e^\Lambda v(z))_+=1-e^{-L_z}$ on the regular set, with
$e^{-\infty}=0$. The extended convention is essential at a zero void. Its expectation vanishes exactly when $L_z\to0$ in target probability. This is not uniform control of every Palm void, and it does not result from dividing an absolute approximation by $e^{-\Lambda}$.
...
Two other exact formulations are useful for separating sufficient conditions from the concluded deficit.
Cumulants below are defined by finite moment--partition identities.
Deterministic recentering leaves orders at least two unchanged, but
$\operatorname{cum}_z(J_{j+1,L}-p)=\EE_zJ_{j+1,L}-p$ at order one.
Since $p$ is a reference rate, not necessarily the planted mean, these
singleton terms must be kept. If $W_j=J_{j+1,L}-p$ and
\[
...
\]
Its probability is $2^{-\pi(M+Q)}$ in the original unconditioned model;
it is not a new admissible conditioning event. The cutoff $M+Q$ covers
the queried vertices and has $Q=o(M)$. A cylinder enlarged to $2M$
would change the witness cost and cannot be substituted here. On this event every queried value is $(-1)^{v_q(m)}$.
...
\[
 T-\lfloor T/q\rfloor-\lfloor(T+1)/q\rfloor
       =(1-2/q)M/q+O(1).
\]
The omissions are counted in $[1,M]$, including the $L$ terminal
indices outside $G\subseteq\{1,\ldots,M-L\}$. Their additional cost
$L=O(\log M)$ is negligible. Omitting $o(M/H)=o(M/q)$ sites leaves at least
