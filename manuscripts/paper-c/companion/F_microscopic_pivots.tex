...
\end{lemma}
\begin{proof}
Write $\lambda=Mp$ for the ambient intensity in the cutoff, whereas
$\Lambda=np$ is the exact target intensity in the hypothesis. Since
$L=O(\log M)$, eventually $\Lambda\le\lambda\le2\Lambda$.
For any fixed $0<c_1<c$, the cost $\log2$ is absorbed by
$(c-c_1)\nu$, giving $I+\log\lambda\le V-c_1\nu$ eventually.
Thus no stronger conditioning hypothesis is introduced. The cutoff gives
$2^{E_*}\ge e^{I+V}(2+\lambda)$ and, in particular,
$e^I\lambda2^{-E_*-1}\le e^{-V}/2$.
The budget also gives $I,\log\Lambda,a_M,E_*=O(V)=o(\log M)$.
...
\cref{lem:stable-lift}. The local bound by four times the rate product
already holds on every small-prime fibre and therefore also after mixing
under $\mathcal A_M$, without that factor. The later common factor
$e^I$ is a valid, coarser way of writing the combined ledger.
...
\]
The first inequality includes $a=b$. For the second, put
$u=w\wedge v$, the coordinatewise minimum. Telescope from $u$ to $w$
and from $u$ to $v$ along coordinate steps. Both paths stay in the
nonnegative lattice and their total length is
$\|w-u\|_1+\|v-u\|_1=\|w-v\|_1$.
Thus comparability of $w$ and $v$ is not required. The conditional-law identity gives
\[
...
The analytic solution and its Hessian bound are the explicit input used
by the associated Lean proof of \cref{supp:pivot:lem:palm}; the semigroup
construction printed above is not reconstructed by that formal proof.
This distinction concerns formal coverage, not an extra hypothesis in
the written coupling argument.

\subsection{A uniform reciprocal-pivot estimate}
For $m\ge1$ define $\kappa_{\rm odd}(m)$ as in the article and put
$C(X,z)=\#\{1\le m\le X:\kappa_{\rm odd}(m)\le z\}$.
For $z\ge1$, the condition is exactly that every valuation at a prime
above $z$ is even, namely the earlier defective-integer predicate.
The integer zero is never included.
...
\end{lemma}
\begin{proof}
A parameter calculation gives the tangent inequality directly.
Put $\nu(t)=e^t/t$ and $D_{\rm par}(t)=e^t-\operatorname{Ei}(t)$
for $t>1$. At the fixed saddle parameter $u$,
\[
 \frac{d}{dt}\{D_{\rm par}(t)-u\nu(t)\}
       =\frac{e^t(t-1)(t-u)}{t^2}.
\]
This expression is nonpositive up to $u$ and nonnegative afterwards,
so the bracket has its minimum at $u$. Since
$D_{\rm par}(u)=V$ and $\nu(u)=\nu$, evaluation at the parameter
corresponding to $H/w$ gives
$D(H/w)\ge V+u(H/w-\nu)$, without differentiating an inverse function.
...
\[
 ...
 \le X\exp\{-2V+o(\nu)\},
\]
by Lemmas~\ref{supp:pivot:lem:rankin} and \ref{supp:pivot:lem:saddle}.
Indeed $-b-D(H/(b+1))=1-\{b+1+D(H/(b+1))\}$: the next-slice
shift costs the fixed factor $e$ before absorption in $o(\nu)$.
For an integer pivot, $\kappa_{\rm odd}>\lfloor e^V\rfloor$ is
exactly $\kappa_{\rm odd}>e^V$; no threshold mass is lost. There are $O(V)$ intervals, with $\log V=o(\nu)$. Their uniform remainders can be summed. The omitted tail is smaller since $V/\nu\to\infty$.
\end{proof}
...
For this privacy assertion alone, exceeding $Q$ suffices: a common
prime divisor of two distinct vertices would divide their nonzero
difference of size at most $Q$. Odd valuation, not valuation exactly
one, supplies the parity coordinate. The stronger $Y>2(Q+1)$ is
retained for the separate local-pair argument.

The finite cylinder must include every prime up to an integer
$R\ge\max\{Y,n+Q\}$. This covers all marked vertices and the entire
conditioning event, not only the terminal prefix $M$. Restricting the
infinite prime-sign source to this cylinder preserves the conditional
laws of all the finite events below. Define
...
\end{proposition}
\begin{proof}
For a fixed prime and target offset $b$, at most $n/q+1$ grid indices satisfy the divisibility; no bound on $b/q$ is required. Sum over offsets and selected primes. When summing reciprocals over $j,a$, each raw integer $m=j+a$ occurs at most $Q+1$ times. Apply Proposition~\ref{supp:pivot:prop:reciprocal}, use $X\asymp n$, and absorb $\log(Q+1)=O(\log H)=o(\nu)$.
\end{proof}
...
\begin{equation}\label{supp:pivot:eq:forcing}
 ...
 \qquad 0\le a\le L+e+1.
\end{equation}
Every other bit is left unchanged, including the pivots with
$L+e+2\le a\le Q$ in the maximal support. Conditional on all the
nonpivot bits and on $\mathcal A_M$, the distinct pivot bits are
independent and fair. Thus the event has probability
...
\[
 ...
 \le p^2\gamma_\alpha\gamma_{\alpha'}
       2^{\rho^{\rm val}_{Q+1}(j+1,k+1)}.
\]
Bound its conditional version by $e^I$ times this nonnegative quantity, split $2^\rho=1+(2^\rho-1)$, and enlarge only the excess to all separated pairs. The constant term
stays on the directed change sets; extending it to all pairs would add
an unnecessary quadratic baseline. Since $\sum_{\alpha\in\mathcal T_*}\gamma_\alpha\le1$, this proves
