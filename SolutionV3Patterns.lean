...
namespace Patterns
open Analysis
 theorem maskedField_eq (N L Y : ℕ) (mask : Finset ℕ) (W : Finset (Fin (L+1) → Bit))
    (sigma : SmallSample (2*N+L) Y) : maskedField N L Y mask W sigma =
    PaperC.V282.ProcessAGGInput.indicatorField
      (PaperC.V282.DictionaryFieldModel.maskedWordIndicator N L Y W mask sigma) := by
  funext eta i
  unfold maskedField PaperC.V282.ProcessAGGInput.indicatorField
    PaperC.V282.DictionaryFieldModel.maskedWordIndicator
    PaperC.V282.DictionaryFieldModel.conditionedWordIndicator
    PaperC.V282.DictionaryFieldModel.finiteWordIndicator
  by_cases hm : i.1.val∈mask
  · simp only [hm,ite_true,true_and,decide_eq_true_eq]
    rfl
  · simp [hm]
 theorem maskedRates_eq (N L : ℕ) (mask : Finset ℕ) (W : Finset (Fin (L+1) → Bit)) :
    maskedRates N L mask W = PaperC.V282.DictionaryFieldTransfer.allWordRates N L W mask := by
  funext i
  unfold maskedRates PaperC.V282.DictionaryFieldTransfer.allWordRates
  split_ifs <;> rfl
 theorem maskedMeanDistance_eq (N L Y : ℕ) (mask : Finset ℕ) :
    maskedMeanDistance N L Y mask = PaperC.Prel8.TypicalDictionaryTransfer.maskedDistance N L Y mask := by
  funext W
  unfold maskedMeanDistance PaperC.Prel8.TypicalDictionaryTransfer.maskedDistance
  simp only [maskedField_eq,maskedRates_eq]
  rfl

 theorem typicalRate_eq (K epsilon eta : ℝ) (N : ℕ) : typicalRate K epsilon eta N =
    PaperC.Prel8.TypicalDictionaryTheorem.rate K epsilon eta N := by
  unfold typicalRate PaperC.Prel8.TypicalDictionaryTheorem.rate
  rw [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq]
end Patterns
...
theorem typical_dictionary_mean (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
  ...
  simp only [maskedMeanDistance_eq,typicalRate_eq,Analysis.hardCutoff_eq]
  exact PaperC.Prel8.TypicalDictionaryTheorem.mean_rate_eventually (Process.process_input_to_core hAGG) hPNT
    betaMin betaMax K epsilon eta hbetaMin hbeta hK hepsilon heta
...
theorem affine_dictionary_mean (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
  ...
  simp only [maskedMeanDistance_eq,typicalRate_eq,Analysis.hardCutoff_eq]
  exact PaperC.Prel8.AffineDictionaryTheorem.mean_rate_eventually (Process.process_input_to_core hAGG) hPNT
    betaMin betaMax K epsilon eta hbetaMin hbeta hK hepsilon heta
