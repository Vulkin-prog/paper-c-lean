...
/-!
# Paper C external-audit solution: infinite-product transfer

This file reproduces the infinite-model interface of `ChallengeTransfer.lean`
without importing that Challenge.  It then closes the exact identity between
the source infinite Rademacher law and the finite cylinder by the frozen
Paper C endpoint.  This remains a separate Comparator target: success of the
finite theorem alone does not establish the identity.
...
theorem paper_c_theorem_one_one_infinite_finite_law_identity
    ...
    PaperCAudit.InfiniteStartProbabilityTransfer.infiniteDyadicStartLaw N L =
      PaperCAudit.SectionThirteenCouplings.fullDyadicStartLaw N L := by
  exact
    PaperC.InfiniteStartProbabilityTransfer.infiniteDyadicStartCount_law_eq_fullDyadicStartLaw
      N L
