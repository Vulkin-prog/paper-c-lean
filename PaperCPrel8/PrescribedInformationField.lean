...
/-! # The full signed field at an independently prescribed admissible cutoff -/
namespace PaperC.Prel8.PrescribedInformationField
...
open PrescribedInformationBudget InformationBudget
...
theorem prescribed_field_in_band (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
  ...
  obtain ⟨Nz,hz⟩ := eventually_atTop.1
    ((hlog.eventually (eventually_ge_atTop (max H1 (max H2 (max (saddleThreshold 1) (saddleThreshold 2)))))).and
      ((polynomial_absorption epsilon hepsilon).and (free_tail_absorption c' hc')))
  refine ⟨max 2 (max Nf (max Ne Nz)),?_⟩
  intro N hN L w hwV hwHat hLlo hLhi hlambda A hpos hIV hA hbudget
  ...
  have hw : 0 < w :=
    (saddleCutoff_pos (by norm_num : (0:ℝ)<1) ((le_max_left _ _).trans hH)).trans_le hwV
  ...
  have hwlo : lo*Real.sqrt (Real.log N*Real.log (Real.log N)) ≤ w :=
    (h1 _ ((le_max_left _ _).trans hdata.1)).1.trans hwV
  have hwhi : w ≤ hi*Real.sqrt (Real.log N*Real.log (Real.log N)) :=
    hwHat.trans (h2 _ ((le_max_left _ _).trans ((le_max_right _ _).trans hdata.1))).2
  have hraw := hf N (by omega) w hwlo hwhi L (movableMarkCutoff N) hLlo
    (he N (by omega) L hLhi) A hA hpos
  have hproducts := free_exponential_products hI hlambda hnu hc' hcc hbudget
  have hpoly := hdata.2.1
  have hwexp : Real.exp w ≤ Real.exp (saddleCutoff 2 (Real.log N)) := Real.exp_le_exp.mpr hwHat
  ...
  have hsmall := ht.trans (hdata.2.2 w hwV)
...
end PaperC.Prel8.PrescribedInformationField
