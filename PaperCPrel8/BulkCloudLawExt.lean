...
namespace PaperC.Prel8.BulkCloudLawExt

open MeasureTheory ProbabilityTheory V282.BulkMarkedTypes V282.ExactMarkedModel V282.BulkSupportGraph

noncomputable section

theorem project_eq_iff {E : ℕ} {N : Finset ℕ} (config target : SpatialMarkedConfig N) :
...
theorem configuration_eq_iff_all_projections {N : Finset ℕ} (config target : SpatialMarkedConfig N) :
...
def projectionFiber (N : Finset ℕ) (E : ℕ) (target : SpatialMarkedConfig N) : Set (SpatialMarkedConfig N) :=
  {config | projectConfiguration N E config=projectConfiguration N E target}

theorem projectionFiber_antitone (N : Finset ℕ) (target : SpatialMarkedConfig N) :
...
theorem singleton_eq_iInter_projectionFiber (N : Finset ℕ) (target : SpatialMarkedConfig N) :
...
theorem spatialMarkedMeasure_ext {N : Finset ℕ} (mu nu : Measure (SpatialMarkedConfig N))
  ...
  congr 1
  funext E
  have he := congrArg (fun m : Measure (LabelledIndex N (Fin (E+1) × F₂) → ℕ) =>
...
theorem hasLaw_of_all_projections {Omega : Type*} [MeasurableSpace Omega] {N : Finset ℕ}
...
theorem hasLaw_of_matching_projection_laws {Omega : Type*} [MeasurableSpace Omega] {N : Finset ℕ}
    (mu : Measure Omega) (nu : Measure (SpatialMarkedConfig N))
    [IsFiniteMeasure mu] [IsFiniteMeasure nu]
    (X : Omega → SpatialMarkedConfig N) (hX : Measurable X)
    (laws : ∀ E : ℕ, Measure (LabelledIndex N (Fin (E+1) × F₂) → ℕ))
...
end PaperC.Prel8.BulkCloudLawExt
