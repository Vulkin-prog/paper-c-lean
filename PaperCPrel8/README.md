...
| `PrescribedInformationBudget` | The field error budget at every admissible cutoff, not only the crossing |
| `PrescribedInformationField` | The full signed field at an independently prescribed admissible cutoff |
| `PrescribedInformationPaper` | The paper's information-adapted labelled comparison, without extra band assumptions |
| `PrimeWeightedLimit` | The fixed-t witness threshold on the original arithmetic law |
| `PrimeWeightedWitness` | The arithmetic prime witness for every fixed positive polynomial weight |
| `TiltedVoidDerivative` | Exact logarithmic derivative of the finite tilted void polynomial |
