...
namespace Limits
theorem startCount_eq (N L : ℕ) : startCount N L =
    PaperC.InfiniteStartProbabilityTransfer.infiniteDyadicStartCount N L := rfl
theorem actualJointVector_eq {R : Type*} [Fintype R] [DecidableEq R]
    (N L J : ℕ) (excess : R → ℕ) : actualJointVector N L J excess =
    PaperC.V282.PoissonGaussianSource.actualJointVector N L J excess := rfl
theorem spatialMarkedSource_eq (N L : ℕ) (omega : InfiniteSample) : spatialMarkedSource N L omega =
    PaperC.V282.SpatialMarkedSource.spatialMarkedSource N L omega := by
  have heq (j : Fin N × (ℕ × F₂)) :
      (PaperC.V282.SpatialMarkedSource.spatialMarkedSource N L omega) j=spatialMarkedValue N L omega j := by
    rw [PaperC.V282.SpatialMarkedSource.spatialMarkedSource_apply]
    unfold PaperC.V282.SpatialMarkedSource.spatialMarkedValue
      PaperC.V282.ExactMarkedModel.signedMarkValue spatialMarkedValue
    exact if_congr Iff.rfl rfl rfl
  have hex : ∃ c : SpatialMarkedConfig N, ∀ j, c j=spatialMarkedValue N L omega j :=
    ⟨PaperC.V282.SpatialMarkedSource.spatialMarkedSource N L omega, heq⟩
  ext j
  exact (Classical.epsilon_spec hex j).trans (heq j).symm
theorem centeredSourceMeasure_eq (N : ℕ) (omega : InfiniteSample) : centeredSourceMeasure N omega =
    PaperC.V282.D4ClosureLaplaceSource.centeredSourceMeasure N omega := by
  unfold centeredSourceMeasure PaperC.V282.D4ClosureLaplaceSource.centeredSourceMeasure
  rw [spatialMarkedSource_eq]; rfl
theorem restrictedCountLaw_eq (N L : ℕ) (C : Set InfiniteSample) : restrictedCountLaw N L C =
    PaperC.V282.RestrictedPoissonTransfer.restrictedCountLaw N L C := rfl
theorem extremeProbability_eq (N L : ℕ) (j : ℤ) : extremeProbability N L j =
    PaperC.V282.D4ClosureExtremeTheorem.extremeProbability N L j := rfl
end Limits

namespace Limits
open PaperC PaperC.V282 MeasureTheory ProbabilityTheory
local instance instMeasurableHalfCountsBridge : MeasurableSpace (ℕ →₀ ℕ) := ⊤

theorem spatialHalfCounts_eq (m : ℤ) : spatialHalfCounts m=
    D4GridIdentificationConfiguration.spatialHalfCounts m := rfl

theorem halfLinePointConfiguration_eq (m : ℤ) : halfLinePointConfiguration m=
    D4ClosureSpatialHalfLines.halfLinePointConfiguration m := rfl

theorem upperPointLaw_eq (theta : ℝ) (m : ℤ) : upperPointLaw theta m=
    (D4ClosureSpatialHalfLines.halfLinePointLaw theta m).val := rfl

theorem poissonHalfPointLaw_eq (theta : ℝ) (m : ℤ) : poissonHalfPointLaw theta m=
    (PoissonPointProcess.poissonPointLaw (D4ClosureIntegerLevels.integerHalfRate theta m)
      D4GridIdentificationTarget.halfMarkMeasure D4ClosureSpatialIdentification.halfContinuousMark
      D4ClosureSpatialIdentification.measurable_halfContinuousMark).val := rfl

theorem fixedHalfPointLaw_eq (n : ℕ) : fixedHalfPointLaw n=
    (D4ClosureSpatialIdentification.fixedHalfPointLaw n D4ClosureSpatialIdentification.halfContinuousMark
      D4ClosureSpatialIdentification.measurable_halfContinuousMark).val := rfl
end Limits
...
theorem paper_c_v3_limits_joint_poisson_gaussian (hStein : Analysis.DirectionalSteinFactorsStatement)
    ...
    ∃ laws : ℕ → ProbabilityMeasure (WithLp 2 (EuclideanSpace ℝ (Fin (J+1)) × EuclideanSpace ℝ R)),
      ...
      Tendsto laws atTop (𝓝 (poissonGaussianTarget J (criticalPoissonRates theta R))) := by
  rw [show Analysis.hardCutoff = PaperC.V282.HardPoissonRates.hardCutoff from funext Analysis.hardCutoff_eq] at hC
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hbudget
  refine ⟨fun n => PaperC.V282.PoissonGaussianSource.conditionalJointLaw (sizes n)
    (movingLength (sizes n) (depths n)) J (criticalExcess R (depths n)) (C n) (hpos n), ?_, ?_⟩
  · intro n
    rw [Limits.actualJointVector_eq]
    rfl
  · exact PaperC.V282.PoissonGaussianTheorem.theorem_five_ten
      (Analysis.directional_input_eq.mp hStein) (Analysis.pnt_input_eq.mp hPNT)
      c hc sizes depths theta R J hsizes hdepths hsmall hphase C hC hpos hbudget
...
theorem paper_c_v3_limits_gaussian_covariance (J : ℕ) (j k : Fin (J+1)) :
    cov[fun x => x j, fun x => x k;
      (gaussianThresholdLaw J : Measure (EuclideanSpace ℝ (Fin (J+1))))] =
        1 / (2 : ℝ)^max j.val k.val := by exact PaperC.V282.GaussianThresholdCovariance.gaussianThresholdLaw_covariance J j k
...
theorem paper_c_v3_limits_ar_covariance (J : ℕ) (j k : Fin (J+1)) :
    cov[standardizedProjection J j, standardizedProjection J k;
      (gaussianThresholdLaw J : Measure (EuclideanSpace ℝ (Fin (J+1))))] =
      (2 : ℝ)^(-|(j.val : ℝ)-(k.val : ℝ)|/2) := by exact PaperC.V282.GaussianThresholdAR.standardizedProjection_covariance J j k
...
theorem paper_c_v3_limits_ar_recursion (J : ℕ) (j : Fin J)
    ...
    standardizedProjection J j.succ x =
      ...
      (2 : ℝ)^(-(1 : ℝ)/2) * innovationProjection J j x := by exact PaperC.V282.GaussianThresholdAR.standardized_ar_recursion J j x
...
theorem paper_c_v3_limits_ar_independent_innovations (J : ℕ) :
    iIndepFun (fun i : Option (Fin J) => arNoiseProjection J i)
      (gaussianThresholdLaw J : Measure (EuclideanSpace ℝ (Fin (J+1)))) := by exact PaperC.V282.GaussianThresholdAR.arNoises_independent J

/-- PaperC.V282.GaussianThresholdAR.hasLaw_arNoise. -/
theorem paper_c_v3_limits_ar_normal_innovations (J : ℕ) (i : Option (Fin J)) :
    HasLaw (arNoiseProjection J i) (gaussianReal 0 1) (gaussianThresholdLaw J : Measure _) := by exact PaperC.V282.GaussianThresholdAR.hasLaw_arNoise J i
...
theorem paper_c_v3_limits_local_stirling_bounds {rate : ℝ} (hr : 0 < rate) {n : ℕ} (hn : 0 < n) :
    exp (-(1 / (12 * n))) * poissonLocalApprox rate n ≤
      ...
      exp (-rate) * rate ^ n / n.factorial ≤ poissonLocalApprox rate n := by exact PaperC.V282.PoissonStirlingBounds.poisson_local_bounds hr hn
...
theorem paper_c_v3_limits_local_stirling_relative_error {rate : ℝ} (hr : 0 < rate) {n : ℕ} (hn : 0 < n) :
    |(exp (-rate) * rate ^ n / n.factorial) / poissonLocalApprox rate n - 1| ≤
      1 / (12 * n) := by exact PaperC.V282.PoissonStirlingBounds.poisson_local_relative_error hr hn
...
theorem paper_c_v3_limits_poisson_local_limit (rates : ℕ→ℝ≥0) (n : ℕ→ℕ) (t : ℕ→ℝ) (K : ℝ)
    ...
    Tendsto (fun k => (poissonMeasure (rates k)).real {n k}/gaussianLatticeMass (rates k) (t k))
      atTop (𝓝 1) := by exact PaperC.V282.PoissonQuantitativeCentral.poisson_gaussian_local_ratio_tendsto_one rates n t K hrate ht hround
...
theorem paper_c_v3_limits_poisson_moderate_bound : ∃ C : ℝ, 0<C ∧ ∀ rate : ℝ≥0, (4096 : ℝ)≤rate →
    ...
    |(poissonMeasure rate).real (Ici ⌈(rate : ℝ)+Real.sqrt rate*t⌉₊)/normalTail t-1| ≤
      C*((1+t^3)/Real.sqrt rate) := by
  let C := |PaperC.V282.PoissonQuantitativeModerate.moderateConstant|+1
  refine ⟨C,by dsimp [C]; positivity,?_⟩
  intro rate hr t ht hcube
  have hC : PaperC.V282.PoissonQuantitativeModerate.moderateConstant ≤ C :=
    (le_abs_self _).trans (le_add_of_nonneg_right (by norm_num))
  exact (PaperC.V282.PoissonQuantitativeModerate.poisson_moderate_relative_error rate hr t ht hcube).trans
...
theorem paper_c_v3_limits_poisson_moderate_limit (rates : ℕ→ℝ≥0) (t : ℕ→ℝ)
    ...
    Tendsto (fun k => (poissonMeasure (rates k)).real
      (Ici ⌈(rates k : ℝ)+sqrt (rates k)*t k⌉₊)/normalTail (t k)) atTop (𝓝 1) := by exact PaperC.V282.PoissonQuantitativeModerateLimit.poisson_moderate_ratio_tendsto_one rates t hrate ht htnonneg
...
theorem paper_c_v3_limits_poisson_gaussian_cdf_bound : ∃ C : ℝ, 0<C ∧ ∀ rate : ℝ≥0, 0<rate → ∀ t : ℝ,
    |(poissonMeasure rate).real {n : ℕ | ((n : ℝ)-(rate : ℝ))/Real.sqrt rate≤t} -
      (gaussianReal 0 1).real (Iic t)| ≤ C/Real.sqrt rate := by
  refine ⟨PaperC.V282.PoissonQuantitativeBerry.berryConstant,
    lt_of_lt_of_le (by norm_num) PaperC.V282.PoissonQuantitativeBerry.berryConstant_ge_four,?_⟩
  exact PaperC.V282.PoissonQuantitativeBerry.poisson_gaussian_CDF_error
...
theorem paper_c_v3_limits_hard_central_local (hStein : ScalarSteinFactorsStatement) (hPNT : PrimeNumberTheoremRemainder)
  ...
  rw [show Analysis.hardCutoff = PaperC.V282.HardPoissonRates.hardCutoff from funext Analysis.hardCutoff_eq] at hC
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hbudget
  exact PaperC.V282.HardLocalClosureCentral.hard_central_local_probabilities
    (Analysis.scalar_input_eq.mp hStein) (Analysis.pnt_input_eq.mp hPNT)
    c c' hc' hcc sizes lengths counts C t K hsizes hrate ht hround hC hpos hbudget
...
theorem paper_c_v3_limits_soft_central_local (hStein : ScalarSteinFactorsStatement) (hPNT : PrimeNumberTheoremRemainder)
    ...
    Tendsto (fun k => restrictedCountLaw (sizes k) (lengths k) (C k) (counts k) /
      gaussianLatticeMass (fullRate (sizes k) (lengths k)) (t k)) atTop (𝓝 1) := by
  rw [show Analysis.softCutoff = PaperC.V282.SoftRateAssembly.softCutoff from funext Analysis.softCutoff_eq] at hC
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hbudget
  exact PaperC.V282.SoftLocalClosureCentral.soft_central_local_probabilities
    (Analysis.scalar_input_eq.mp hStein) (Analysis.pnt_input_eq.mp hPNT)
    c c' hc' hcc sizes lengths counts C t K hsizes hrate ht hround hC hpos hbudget
...
theorem paper_c_v3_limits_hard_moderate_tail (hStein : ScalarSteinFactorsStatement) (hPNT : PrimeNumberTheoremRemainder)
    ...
    Tendsto (fun k => (cond infiniteRademacherMeasure (C k)).real
      {omega | ⌈(fullRate (sizes k) (lengths k) : ℝ)+sqrt (fullRate (sizes k) (lengths k))*t k⌉₊≤
        startCount (sizes k) (lengths k) omega}/normalTail (t k)) atTop (𝓝 1) := by
  rw [show Analysis.hardCutoff = PaperC.V282.HardPoissonRates.hardCutoff from funext Analysis.hardCutoff_eq] at hadmissible
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hadmissible
  exact PaperC.V282.PoissonQuantitativeTailRates.hard_conditional_tail_normal_ratio_tendsto_one
    (Analysis.scalar_input_eq.mp hStein) (Analysis.pnt_input_eq.mp hPNT)
    betaMin betaMax c hbetaMin hbeta hc sizes lengths t C hsizes hrate ht hadmissible
...
theorem paper_c_v3_limits_integer_process_moving_laplace (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
    ...
    Tendsto (fun n =>
      ...
      atTop (𝓝 0) := by
  rw [show Analysis.hardCutoff = PaperC.V282.HardPoissonRates.hardCutoff from funext Analysis.hardCutoff_eq] at hA
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hbudget
  simp only [Limits.centeredSourceMeasure_eq]
  exact PaperC.V282.D4ClosureLaplaceTheorem.companion_D4_laplace_moving
    (Process.process_input_to_core hAGG) (Analysis.pnt_input_eq.mp hPNT) sizes hsizes g hg hg0 hgc c hc A hA hpos hbudget
...
theorem paper_c_v3_limits_integer_process_phase_limit (hAGG : ProcessAGGStatement)
  ...
  simp only [Analysis.saddleCutoff_eq,Analysis.saddleNu_eq] at hbudget
  simp only [Limits.centeredSourceMeasure_eq]
  exact PaperC.V282.D4ClosureLaplaceTheorem.companion_D4_laplace_phase_limit
    (Process.process_input_to_core hAGG) (Analysis.pnt_input_eq.mp hPNT) sizes hsizes theta hphase g hg hg0 hgc c hc A hA hpos hbudget
...
theorem paper_c_v3_limits_integer_process_target_laplace (theta : ℝ) (g : ℝ × ℤ → ℝ)
    ...
    (∫ sample : IntegerSpatialSample, exp (-(∫ z, g z ∂integerPointMeasure sample))
      ∂integerSpatialSampleMeasure theta) = laplaceFunctional theta g := by exact PaperC.V282.D4ClosureLaplaceTarget.integer_target_laplace theta g hg hg0 hgc
...
theorem paper_c_v3_limits_integer_process_locally_finite (sample : IntegerSpatialSample)
    (K : Set (ℝ × ℤ)) (hK : IsCompact K) : integerPointMeasure sample K < ⊤ := by exact PaperC.V282.D4ClosurePointMeasure.integerPointMeasure_compact_finite sample K hK
...
theorem paper_c_v3_limits_integer_process_infinite_mass (theta : ℝ) :
    ∀ᵐ sample ∂integerSpatialSampleMeasure theta, integerPointMeasure sample Set.univ=⊤ := by exact PaperC.V282.D4ClosureInfiniteMass.ae_integerPointMeasure_univ_top theta
...
theorem paper_c_v3_limits_integer_process_independent_counts (theta : ℝ) :
    iIndepFun (fun r (counts : ℤ → ℕ) => counts r) (integerCountMeasure theta) := by exact PaperC.V282.D4ClosureIntegerLevels.independent_integer_counts theta
...
theorem paper_c_v3_limits_integer_process_poisson_counts (theta : ℝ) (r : ℤ) :
    HasLaw (fun counts : ℤ → ℕ => counts r) (poissonMeasure (integerLevelRate theta r))
      (integerCountMeasure theta) := by exact PaperC.V282.D4ClosureIntegerLevels.hasLaw_integer_coordinate theta r
...
theorem paper_c_v3_limits_integer_process_finite_upper_tails (theta : ℝ) :
    ∀ᵐ sample ∂integerSpatialSampleMeasure theta,
      ∀ m : ℤ, (Function.support (fun e : ℕ => (sample (m+e)).1)).Finite := by exact PaperC.V282.D4ClosurePointLaws.ae_spatial_upper_tails_finite theta
...
theorem paper_c_v3_limits_integer_process_conditioned_positions (theta : ℝ) (r : ℤ) (n : ℕ)
    ...
    (cond (integerSpatialSampleMeasure theta) {sample | (sample r).1 = n}).map
      ...
      (Measure.dirac n).prod (markSequenceMeasure) := by exact PaperC.V282.D4ClosurePointLaws.conditional_level_positions theta r n hn
...
theorem paper_c_v3_limits_extreme_phase_limit (hStein : ScalarSteinFactorsStatement) (hPNT : PrimeNumberTheoremRemainder)
  ...
  simpa only [Limits.extremeProbability_eq] using
    PaperC.V282.D4ClosureExtremeTheorem.companion_D4_extreme
      (Analysis.scalar_input_eq.mp hStein) (Analysis.pnt_input_eq.mp hPNT)
      sizes lengths theta j hsizes hrate hphase
...
theorem paper_c_v3_limits_d4_half_line_is_whole_restriction (theta : ℝ) (m : ℤ) :
    ∀ᵐ sample ∂Limits.integerSpatialSampleMeasure theta,
      ((Limits.halfLinePointConfiguration m sample).val.map (Limits.upperPosition m))=
        (Limits.integerPointMeasure sample).restrict (Set.univ ×ˢ Set.Ici m) := by
  exact PaperC.V282.D4ClosureWholeRestriction.ae_halfLinePointConfiguration_is_restriction theta m
