...
/--
Conditional second-factorial-moment conclusion of Theorem 1.4, with the
three actual Section 11 interfaces exposed at the public theorem boundary.
-/
theorem factorialMomentError_uniformLittleOOne
    ...
    UniformLittleOOn
      ...
      (fun _ _ => 1) := by
  apply
    factorialMomentError_uniformLittleOOne_of_propositionElevenTwo hC
  exact
...
/--
The conditional second-factorial-moment conclusion in its literal Poisson
form: `E[(Z)₂] - λ² = o_C(1)`.
-/
theorem dyadicSecondFactorialPoissonError_uniformLittleOOne
    {C : ℝ} (hC : 0 ≤ C) (A : ℕ) (hA : 1 ≤ A)
    (smallRowRank : PropositionElevenTwo.SmallRowRankFamily)
    (rankBudget : PropositionElevenTwo.RankBudgetFamily)
    (hhosts : PropositionNineNine.HostCountStatement C A)
    ...
    UniformLittleOOn
      ...
      (fun _ _ => 1) := by
  have hfinite :=
    factorialMomentError_uniformLittleOOne
      hC A hA smallRowRank rankBudget
      hhosts hnonterminal hterminal
  have hbaseline :=
    factorialBaselinePoissonError_uniformLittleOOne hC
  have hsum :=
    PropositionElevenTwo.uniformLittleOOn_add hfinite hbaseline
  have heq :
      dyadicSecondFactorialPoissonError =
        fun N L =>
          factorialMomentError N L +
...
/--
Conditional variance conclusion of Theorem 1.4:
`Var(Z_{N,L}) - λ = o_C(1)`.

As with the factorial moment, the public boundary lists the actual Section 11
interfaces rather than hiding them in a new aggregate statement.
-/
theorem dyadicVariancePoissonError_uniformLittleOOne
    {C : ℝ} (hC : 0 ≤ C) (A : ℕ) (hA : 1 ≤ A)
    (smallRowRank : PropositionElevenTwo.SmallRowRankFamily)
    (rankBudget : PropositionElevenTwo.RankBudgetFamily)
    (hhosts : PropositionNineNine.HostCountStatement C A)
    ...
    UniformLittleOOn
      ...
      (fun _ _ => 1) := by
  have hfactorial :=
    dyadicSecondFactorialPoissonError_uniformLittleOOne
      hC A hA smallRowRank rankBudget
      hhosts hnonterminal hterminal
  have hmean :=
    dyadicFirstMomentPoissonError_uniformLittleOOne hC
  obtain ⟨Nwindow, hNwindow⟩ :=
    CriticalRunWindow.firstMomentWindow_eventually hC
  let B := CriticalRunWindow.balanceConstant C
  have hBnonneg : 0 ≤ B := by
  ...
  have hδ : 0 < δ := by
    dsimp only [δ]
    positivity
  obtain ⟨Nfactorial, hNfactorial⟩ :=
    hfactorial δ hδ
  obtain ⟨Nmean, hNmean⟩ :=
    hmean δ hδ
  obtain ⟨NmeanOne, hNmeanOne⟩ :=
    hmean 1 (by norm_num)
  refine
    ⟨max Nwindow
  ...
  have hmeanBound :
      |dyadicFirstMomentPoissonError N L| ≤ δ := by
    simpa only [abs_one, mul_one] using
      hNmean N ((le_max_left _ _).trans hmeanTail) L hrun
  have hmeanOne :
      |dyadicFirstMomentPoissonError N L| ≤ 1 := by
    simpa only [abs_one, mul_one] using
