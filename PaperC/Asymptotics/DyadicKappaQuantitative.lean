...
/--
The moderate-density part has a fixed power saving, so it is bounded by
the sharper exponential scale of Corollary 11.3 for every fixed
nonnegative saving constant.
-/
private theorem moderateDensityMass_dyadic_uniformBigO_quantitative
    {C K : ℝ} (hostEnvelope : ℕ → ℕ → ℝ)
    (hhostRate :
      UniformLinearSubpolynomialOn
        (CriticalRunWindow.InRunLengthWindow C) hostEnvelope)
    ...
    UniformBigOOn
      ...
      (fun N L ↦
        moderateDensityIntrinsicNonterminalMass
          3 K N (2 * N) L)
      (PropositionElevenThree.quantitativeHomogeneousScale c) := by
  let envelope : ℕ → ℕ → ℝ :=
    fun N L =>
      hostEnvelope N L *
        moderateDensityResidualWeightEnvelope N L
  ...
  have henvelopeBigO :
    ...
    PropositionElevenThree.rationalPower_uniformBigO_quantitativeHomogeneousScale
      (by omega) henvelopeRate hc
  apply SectionThirteenRate.uniformBigOOn_mono henvelopeBigO
  obtain ⟨N₁, hN₁⟩ := hhostsTen
  refine ⟨max N₁ 2, ?_⟩
  intro N hN L hrun
...
/--
The high-density part keeps the literal exponential saving transported
from the nonterminal rank gap.
-/
private theorem highDensityMass_dyadic_uniformBigO_quantitative
    {C Cterm K : ℝ} (hC : 0 ≤ C) (hCterm : 0 ≤ Cterm)
    (hthreshold : 2 * Cterm < K * Real.log 2)
    (hhostsTwo :
      ∃ N₁ : ℕ, ∀ N ≥ N₁, ∀ M L,
  ...
  have henvelopeQuantitative :=
    BoundedRatioDenseQuantitative.highDensityIntrinsicNonterminalMassEnvelope_uniformBigO_quantitative
      hC hCterm hthreshold
  apply SectionThirteenRate.uniformBigOOn_mono henvelopeQuantitative
  obtain ⟨N₁, hN₁⟩ := hhostsTwo
  refine ⟨max N₁ 2, ?_⟩
  intro N hN L hrun
  have hNtwo : 2 ≤ N :=
...
/--
Sector six at the exact Corollary 11.3 scale.  The moderate branch has a
fixed `N^(5/3+o(1))` saving and the dense branch supplies the uniform
positive exponential constant used by both contributions.
-/
private theorem nonterminalSector_dyadic_uniformBigO_quantitative
    {C Cterm K : ℝ} (hC : 0 ≤ C) (hCterm : 0 ≤ Cterm)
    (hthreshold : 2 * Cterm < K * Real.log 2)
    (hostEnvelope : ℕ → ℕ → ℝ)
    (hhostRate :
  ...
  have hc :
    ...
    BoundedRatioDenseQuantitative.denseQuantitativeConstant_pos
      hCterm hthreshold
  have hmoderate :=
    moderateDensityMass_dyadic_uniformBigO_quantitative
      (K := K) hostEnvelope hhostRate hhostNonneg hhostsTen hc.le
  have hhigh :=
    highDensityMass_dyadic_uniformBigO_quantitative
      hC hCterm hthreshold hhostsTwo
  have hsum :=
    PropositionElevenThree.uniformBigOOn_add hmoderate hhigh
  simpa only [
...
/-! ## Exact exponential endpoint of Corollary 11.3 -/

/--
Every literal sector of the canonical seven-sector partition has the same
positive exponential saving.  Fixed power savings are transported to the
dense-sector constant, while sector six uses that constant directly.
-/
private theorem everySector_dyadic_uniformBigO_quantitative
    {C Cterm K : ℝ} (hC : 0 ≤ C) (hCterm : 0 ≤ Cterm)
    (hK : 0 ≤ K)
    (hthreshold : 2 * Cterm < K * Real.log 2)
    (hES :
  ...
  | manyDefects =>
      ...
      exact
        PropositionElevenThree.rationalPower_uniformBigO_quantitativeHomogeneousScale
          (by omega) hdyadic hc.le
  | nonterminal =>
      exact
        nonterminalSector_dyadic_uniformBigO_quantitative
          hC hCterm hthreshold hostEnvelope
            hhostRate hhostNonneg hhostsTen hhostsTwo
  | terminal =>
      exact
...
/--
The bounded-ratio mother mass at `M = 2N`, with exactly the exponential
scale printed in Corollary 11.3.
-/
private theorem R2κ_dyadic_uniformBigO_quantitative
    {C Cterm K : ℝ} (hC : 0 ≤ C) (hCterm : 0 ≤ Cterm)
    (hK : 0 ≤ K)
    (hthreshold : 2 * Cterm < K * Real.log 2)
    (hES :
      ...
      UniformBigOOn
        ...
        (PropositionElevenThree.quantitativeHomogeneousScale
          (BoundedRatioDenseQuantitative.denseQuantitativeConstant
            Cterm K)) :=
    PropositionElevenThree.uniformBigOOn_fintype_sum
      (everySector_dyadic_uniformBigO_quantitative
        hC hCterm hK hthreshold hES hPell hostEnvelope
          hhostRate hhostNonneg hhostsTen hhostsTwo)
  have htotal :=
    PropositionElevenThree.uniformBigOOn_add
...
/--
Canonical source-facing form of Corollary 11.3.  It exposes a uniform
positive constant `c_R` and concludes exactly

`R₂(N,L) ≪_C N² exp (-c_R sqrt(log N) / log log N)`

through the repository's fully quantified `UniformBigOOn` predicate.
-/
theorem corollary_eleven_three_canonical
    {C : ℝ} (hC : 0 ≤ C)
    (hES :
      EvertseSilvermanInput.EvertseSilvermanAbscissaStatement)
    (hConductor :
  ...
  let c_R : ℝ :=
    BoundedRatioDenseQuantitative.denseQuantitativeConstant
      Cterm K
  have hc_R : 0 < c_R := by
    simpa only [c_R] using
      BoundedRatioDenseQuantitative.denseQuantitativeConstant_pos
        hCterm hthreshold
  have hR2κ :=
    R2κ_dyadic_uniformBigO_quantitative
      hC hCterm hK hthreshold hES hPell hostEnvelope
        hhostRate hhostNonneg hhostsTen hhostsTwo
  have heq :
      PropositionElevenTwo.homogeneousMass 3 =
