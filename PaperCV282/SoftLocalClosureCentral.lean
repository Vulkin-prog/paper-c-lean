...
/-! # The soft central local regime of companion D.1, including bounded rounding -/
namespace PaperC.V282.SoftLocalClosureCentral

open MeasureTheory ProbabilityTheory Filter Topology Real
open InfiniteRademacher InfiniteCylinderTransfer
open AllStartSoftPoisson RareConditioningRates SoftRateAssembly RestrictedPoissonTransfer
open SoftLocalClosureBudget SoftLocalClosureRates PoissonStirlingBounds PoissonCentralAsymptotics
...
theorem soft_central_cost_budget_eventually (c c' : ℝ) (hcc : c'<c) :
    ∀ᶠ N : ℕ in atTop, ∀ I rate t : ℝ, ∀ n : ℕ, 1≤rate →
      2*I+2*log rate+t^2 ≤ saddleCutoff 2 (log N)-c*saddleNu 2 (log N) →
      |log (n : ℝ)-log rate|≤1 →
      |rate*poissonEntropy (n/rate)-t^2/2|≤1 →
      softLocalCost I rate n ≤ saddleCutoff 2 (log N)-c'*saddleNu 2 (log N) := by
  ...
  unfold softLocalCost
  rw [max_eq_right (log_nonneg hr)]
  linarith [(abs_le.mp hl).2,(abs_le.mp he).2]

theorem soft_central_common_band_eventually :
    ∀ᶠ N : ℕ in atTop, ∀ L : ℕ, 1 ≤ (fullRate N L : ℝ) →
      log (fullRate N L : ℝ) ≤ saddleCutoff 2 (log N) →
  ...
  have hvle : saddleCutoff 2 (log N) ≤ log N / 2 := by
...
theorem soft_central_local_probabilities
  ...
  have hadmissible : ∀ᶠ k in atTop,
      ...
      softLocalCost (eventInformation (C k)) (fullRate (sizes k) (lengths k)) (counts k) ≤
        saddleCutoff 2 (log (sizes k))-c'*saddleNu 2 (log (sizes k)) := by
    filter_upwards [hsizes.eventually (soft_central_cost_budget_eventually c c' hcc),
      hsizes.eventually soft_central_common_band_eventually,
    ...
    have hn : 0<counts k := by
      ...
      linarith
    have hlogle : log (fullRate (sizes k) (lengths k) : ℝ)≤saddleCutoff 2 (log (sizes k)) := by
    ...
    exact ⟨hlo,hhi,hmeas,hpositive,hn,
      hcost (eventInformation (C k)) (fullRate (sizes k) (lengths k)) (t k) (counts k)
        hr hb hel.le hee.le⟩
  have hlocal := soft_local_ratio_tendsto_one hStein hPNT lowerConstant upperConstant c'
...
end PaperC.V282.SoftLocalClosureCentral
