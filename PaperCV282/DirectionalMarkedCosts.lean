...
namespace PaperC.V282.DirectionalMarkedCosts
...
open DictionaryMarginalCap TwoWindowParity RelationProfileRestriction WeightedLabelledCosts
open DirectionalSteinComparison LabelledSupportGraph DictionaryFieldFirstCost
...
theorem local_signed_joint_le_four {C N L E Y x y : ℕ} {mask : Finset ℕ}
...
theorem weightedSignedPair_local_le {C N L E Y x y : ℕ} {mask : Finset ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hC : dyadicCutoff N (L+E+1) ≤ C)
    (hY : 2*(L+E+2) ≤ Y) (hmask : mask ⊆ dyadicBlock N)
    (hx : x ∈ fullGoodMask N (L+E+1) Y mask)
    (hy : y ∈ fullGoodMask N (L+E+1) Y mask)
  ...
  have h := mul_le_mul_of_nonneg_left
    (local_signed_joint_le_four hN hL hC hY hmask hx hy hxy hd sigma a b) (hw a b)
...
theorem sum_near_weightedSignedPair_le {C N L E Y : ℕ} {mask : Finset ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hC : dyadicCutoff N (L+E+1) ≤ C)
    (hY : 2*(L+E+2) ≤ Y) (hmask : mask ⊆ dyadicBlock N) (sigma : SmallSample C Y)
    (w : (Fin (E+1) × F₂) → (Fin (E+1) × F₂) → ℝ) (hw : ∀ a b, 0 ≤ w a b) :
    (∑ xy ∈ nearMarkedPairs (L+E+1) (fullGoodMask N (L+E+1) Y mask),
      weightedSignedPair C L E Y sigma xy.1 xy.2 w) ≤
      signedRatePairSum L E w * (8*(N : ℝ)*(L+E+2)) := by
  have hg : fullGoodMask N (L+E+1) Y mask ⊆ dyadicBlock N :=
    fun x hx => hmask (fullGoodMask_subset_mask N (L+E+1) Y mask hx)
  have hsum : (∑ xy ∈ nearMarkedPairs (L+E+1) (fullGoodMask N (L+E+1) Y mask),
      weightedSignedPair C L E Y sigma xy.1 xy.2 w) ≤
      ∑ _xy ∈ nearMarkedPairs (L+E+1) (fullGoodMask N (L+E+1) Y mask),
        4*signedRatePairSum L E w := by
    apply Finset.sum_le_sum
    intro xy hxy
    obtain ⟨hp,hd⟩ := Finset.mem_filter.mp hxy
    obtain ⟨hx,hy,hne⟩ := Finset.mem_offDiag.mp hp
    exact weightedSignedPair_local_le hN hL hC hY hmask hx hy hne hd sigma w hw
...
theorem average_sum_far_weightedSignedPair_le {C N L E Y : ℕ} {mask : Finset ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hmask : mask ⊆ dyadicBlock N)
    (w : (Fin (E+1) × F₂) → (Fin (E+1) × F₂) → ℝ) (hw : ∀ a b, 0 ≤ w a b) :
  ...
  have hpoint (xy : ℕ × ℕ) (hxy : xy ∈ far) :
    ...
    exact average_weightedSignedPair_le
      (by have h := two_le_of_mem_dyadicBlock hN (hmask hp.1); omega)
      (by have h := two_le_of_mem_dyadicBlock hN (hmask hp.2.1); omega) hL w hw
...
theorem typedBOne_signed_le {C N L E Y : ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hC : dyadicCutoff N (L+E+1) ≤ C)
    ...
    typedBOne (largeUniformPMF C Y)
      ...
      Prod.snd (labelledGraph N (L+E+1) Y (Fin (E+1) × F₂)) w ≤
      signedRatePairSum L E w * (2*(N : ℝ)*(L+E+2)+(maskedSupportEdges (L+E+1) Y mask).card) := by
  ...
  have hclosed : ((closedDictionarySitePairs (L+E+1) Y (fullGoodMask N (L+E+1) Y mask)).card : ℝ) ≤
      2*(N : ℝ)*(L+E+2)+(maskedSupportEdges (L+E+1) Y (fullGoodMask N (L+E+1) Y mask)).card := by
    exact_mod_cast card_closedDictionarySitePairs_le (fullGoodMask N (L+E+1) Y mask)
      ((fullGoodMask_subset_mask _ _ _ _).trans hmask)
  have hedge : ((maskedSupportEdges (L+E+1) Y (fullGoodMask N (L+E+1) Y mask)).card : ℝ) ≤
      (maskedSupportEdges (L+E+1) Y mask).card := by
    exact_mod_cast Finset.card_le_card (maskedSupportEdges_mono (fullGoodMask_subset_mask _ _ _ _))
  linarith

/-- Exact regrouping of the weighted second cost on the two genuine site-pair sets. -/
theorem typedBTwo_signed_eq_near_add_far {C N L E Y : ℕ}
  ...
  · rw [weightedLabelPairMass,labelledNeighborPairs_eq_near_union_far,
...
theorem average_typedBTwo_signed_le {C N L E Y : ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hC : dyadicCutoff N (L+E+1) ≤ C)
    ...
    finiteUniformAverage (fun sigma : SmallSample C Y => typedBTwo (largeUniformPMF C Y)
      (maskedLabelledFamily N (conditionedSignedAt C L E Y sigma) (fullGoodMask N (L+E+1) Y mask))
      Prod.snd (labelledGraph N (L+E+1) Y (Fin (E+1) × F₂)) w) ≤
      signedRatePairSum L E w * (8*(N : ℝ)*(L+E+2)+(maskedSupportEdges (L+E+1) Y mask).card+
        (valueWeightMass C (L+E+1) (separatedPairs mask (L+E+1)) : ℝ)) := by
  ...
  have hn := finiteUniformAverage_mono (fun sigma =>
    sum_near_weightedSignedPair_le hN hL hC hY hmask sigma w hw)
  have hnear : finiteUniformAverage (fun sigma : SmallSample C Y =>
      ∑ xy ∈ nearMarkedPairs (L+E+1) (fullGoodMask N (L+E+1) Y mask),
        weightedSignedPair C L E Y sigma xy.1 xy.2 w) ≤
      signedRatePairSum L E w*(8*(N : ℝ)*(L+E+2)) := by
    simpa [finiteUniformAverage,Fintype.card_ne_zero] using hn
  have hfar := average_sum_far_weightedSignedPair_le (C := C) (Y := Y) hN hL hmask w hw
...
theorem average_typedCost_signed_le {C N L E Y : ℕ}
    (hN : 2 ≤ N) (hL : 1 ≤ L) (hC : dyadicCutoff N (L+E+1) ≤ C)
    (hY : 2*(L+E+2) ≤ Y) (mask : Finset ℕ) (hmask : mask ⊆ dyadicBlock N)
    (w : (Fin (E+1) × F₂) → (Fin (E+1) × F₂) → ℝ) (hw : ∀ a b, 0 ≤ w a b) :
    finiteUniformAverage (fun sigma : SmallSample C Y => typedCost (largeUniformPMF C Y)
      (maskedLabelledFamily N (conditionedSignedAt C L E Y sigma) (fullGoodMask N (L+E+1) Y mask))
      Prod.snd (labelledGraph N (L+E+1) Y (Fin (E+1) × F₂)) w) ≤
      signedRatePairSum L E w * (10*(N : ℝ)*(L+E+2)+2*(maskedSupportEdges (L+E+1) Y mask).card+
        (valueWeightMass C (L+E+1) (separatedPairs mask (L+E+1)) : ℝ)) := by
  unfold typedCost
  rw [finiteUniformAverage_add]
  have hn := finiteUniformAverage_mono (fun sigma =>
    typedBOne_signed_le (E := E) (Y := Y) hN hL hC (by omega) mask hmask sigma w hw)
  have hone : finiteUniformAverage (fun sigma : SmallSample C Y => typedBOne (largeUniformPMF C Y)
      (maskedLabelledFamily N (conditionedSignedAt C L E Y sigma) (fullGoodMask N (L+E+1) Y mask))
      Prod.snd (labelledGraph N (L+E+1) Y (Fin (E+1) × F₂)) w) ≤
      signedRatePairSum L E w*(2*(N : ℝ)*(L+E+2)+(maskedSupportEdges (L+E+1) Y mask).card) := by
    simpa [finiteUniformAverage,Fintype.card_ne_zero] using hn
  have htwo := average_typedBTwo_signed_le hN hL hC hY mask hmask w hw
  exact (add_le_add hone htwo).trans_eq (by ring)

end
end PaperC.V282.DirectionalMarkedCosts
