...
/-!
# A maximal-support graph for arbitrary finite labels
...
namespace PaperC.V282.LabelledSupportGraph
...
@[reducible]
def LabelledIndex (N : ℕ) (κ : Type*) := {x : ℕ // x ∈ dyadicBlock N} × κ

/-- Labels at a site and labels on adjacent maximal supports are neighbours. -/
def labelledGraph (N Q Y : ℕ) (κ : Type*) : SimpleGraph (LabelledIndex N κ) where
...
theorem mem_closedNeighborhood_labelledGraph {κ : Type*} [Fintype κ] [DecidableEq κ]
    (N Q Y : ℕ) (i j : LabelledIndex N κ) :
    j ∈ closedNeighborhood (labelledGraph N Q Y κ) i ↔
...
theorem hasExactDependencyGraph_of_labelled_locality {κ : Type*} [Fintype κ] [DecidableEq κ]
    (C N Q Y : ℕ) (X : LabelledIndex N κ → LargeSample C Y → Bool)
    (hlocal : ∀ i eta theta,
      (∀ q : LargePrimeCoordinate C Y,
        largeCoordinatePrime q ∈ largePrimeCoordinates i.1.val Q Y → eta q = theta q) →
      X i eta = X i theta) :
    HasExactDependencyGraph (largeUniformPMF C Y) X (labelledGraph N Q Y κ) := by
  classical
  ...
  let R : LargeSample C Y → Prop := fun eta => HasOutsidePattern X
    (labelledGraph N Q Y κ) alpha pattern eta
  ...
  · intro eta
    ...
    have hi (beta : OutsideIndex (labelledGraph N Q Y κ) alpha) : X beta.val eta = X beta.val theta := by
...
def maskedLabelIndicator {κ S : Type*} (N : ℕ) (X : LabelledIndex N κ → S → Bool)
    (mask : Finset ℕ) : LabelledIndex N κ → S → Bool := by
  classical
...
theorem maskedLabelIndicator_locality {κ : Type*} [Fintype κ] [DecidableEq κ]
    ...
    ∀ i eta theta,
      (∀ q : LargePrimeCoordinate C Y,
        largeCoordinatePrime q ∈ largePrimeCoordinates i.1.val Q Y → eta q = theta q) →
      maskedLabelIndicator N X mask i eta = maskedLabelIndicator N X mask i theta := by
  intro i eta theta h
...
end PaperC.V282.LabelledSupportGraph
