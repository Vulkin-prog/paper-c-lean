...
namespace PaperC.V282.SignedAggregateRates

open ExactMarkedArithmeticRates DictionaryArithmeticRates FullBandArithmetic DyadicFullValueProfile
open SignedAggregateArithmetic SignedDirectionalFactors MaskedArithmeticGeometry MaskedPairGeometry
...
/-- The signed C.1 ledger keeps the maximal full-value profile after directional summation. -/
def signedAggregateLedger (N L E Y : ℕ) (mask : Finset ℕ) : ℝ :=
...
theorem signedAggregateLedger_nonneg (N L E Y : ℕ) (mask : Finset ℕ) :
    0 ≤ signedAggregateLedger N L E Y mask := by
...
theorem signed_ledger_numeric_le {lambda ell F H P m d a g r s : ℝ}
    ...
    m+2*d+s*(10*a+2*g+r) ≤ 32*(lambda*ell*H+F*lambda^2*P) := by
...
end PaperC.V282.SignedAggregateRates
