...
/-! # The labelled hard clause of the macroscopic transfer theorem
...
-/
namespace PaperC.V282.MacroTransportHardBudget
...
open LabelledInformationBudget GrowingLevelParameters RareConditioningRates
...
/-- Uniform information estimate before the length and the actual arithmetic event. -/
theorem hard_labelled_event_bound (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
    ...
    ∃ Mzero : ℕ, ∀ M≥Mzero, ∀ L : ℕ,
      ...
      labelledLogCost (eventInformation A) (fullRate M L)≤
  ...
  obtain ⟨Nb,hb⟩ := hard_restored_budget_eventually c (betaMin*Real.log 2/8) hc hd
  refine ⟨max Nf Nb,?_⟩
  intro M hM L hlo hhi hrate A hA hpos hbudget
  have hraw := hf M (by omega) L hlo hhi A hA hpos
  have hnum := hb M (by omega) (eventInformation A) (fullRate M L) hrate hbudget
...
theorem hard_labelled_sequence_tendsto_zero
    ...
    (hbudget : ∀ᶠ n in atTop,
      labelledLogCost (eventInformation (A n)) (fullRate (sizes n) (lengths n))≤
  ...
  obtain ⟨Mzero,hMzero⟩ := hard_labelled_event_bound hAGG hPNT hLS hShorey hNR
    betaMin betaMax c hbetaMin hbeta hc
  have hd : 0<betaMin*Real.log 2/8 := by have := Real.log_pos (by norm_num : (1 : ℝ)<2);positivity
  have hz := (hard_restored_error_tendsto_zero c (betaMin*Real.log 2/8) hc hd).comp hsizes
  have he : betaMin*Real.log 2/8/2=betaMin*Real.log 2/16 := by ring
  rw [he] at hz
...
/-- The hard labelled information margin in 7.6, for arbitrary growing population sequences. -/
theorem theorem_seven_six_labelled_hard_sequences
    (hAGG : ProcessAGGStatement) (hPNT : PrimeNumberTheoremRemainder)
    (hLS : UniformPrimeDivisorStatement) (hShorey : ShoreySquareProductStatement)
  ...
  apply hard_labelled_sequence_tendsto_zero hAGG hPNT hLS hShorey hNR lo hi c hlo (hlo'.trans hhi) hc
...
end PaperC.V282.MacroTransportHardBudget
