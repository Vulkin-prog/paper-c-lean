...
/-! # Exact directional label costs on an arbitrary finite population -/
namespace PaperC.V282.MacroAggregateWeightedCosts

open BulkSupportGraph BulkProcessCosts DictionaryFieldFirstCost AllStartFieldCosts
...
theorem typedBOne_labelled_eq {κ S : Type*} [Fintype κ] [DecidableEq κ] [Fintype S]
    ...
    (mask : Finset ℕ) (hmask : mask ⊆ sites) (rate : κ → ℝ) (weight : κ → κ → ℝ)
    (hm : ∀ i : LabelledIndex sites κ,
      marginal μ (maskedLabelledFamily sites X mask) i = if i.1.val ∈ mask then rate i.2 else 0) :
    typedBOne μ (maskedLabelledFamily sites X mask) Prod.snd (labelledGraph sites Q Y κ) weight =
  ...
  have hb : typedBOne μ (maskedLabelledFamily sites X mask) Prod.snd (labelledGraph sites Q Y κ) weight =
...
theorem typedBTwo_labelled_eq {κ S : Type*} [Fintype κ] [DecidableEq κ] [Fintype S]
    (μ : FinitePMF S) (X : ℕ → κ → S → Bool) (sites : Finset ℕ) (Q Y : ℕ)
    (mask : Finset ℕ) (hmask : mask ⊆ sites) (weight : κ → κ → ℝ)
    (hdisj : ∀ x ∈ sites, ∀ a b : κ, a ≠ b → ∀ omega,
      ¬(X x a omega = true ∧ X x b omega = true)) :
    typedBTwo μ (maskedLabelledFamily sites X mask) Prod.snd (labelledGraph sites Q Y κ) weight =
  ...
  have ht (i j : LabelledIndex sites κ) :
      (if j ∈ (closedNeighborhood (labelledGraph sites Q Y κ) i).erase i then
        weight i.2 j.2 * jointMarginal μ (maskedLabelledFamily sites X mask) i j else 0) =
  ...
  have hs (i : LabelledIndex sites κ) :
      (∑ j ∈ (closedNeighborhood (labelledGraph sites Q Y κ) i).erase i,
        weight i.2 j.2 * jointMarginal μ (maskedLabelledFamily sites X mask) i j) =
...
end PaperC.V282.MacroAggregateWeightedCosts
