...
# Landrun needs an explicit outer `--` before the sandboxed command.
landrun_binary=${PALOMAR_LANDRUN_BIN:?PALOMAR_LANDRUN_BIN must name the pinned Landrun binary}
landrun_options=()

# Refuse flags that would disable a complete class of Landlock restrictions,
# and fail closed if Comparator starts requesting an option not reviewed here.
while [ "$#" -gt 0 ]; do
  case "$1" in
